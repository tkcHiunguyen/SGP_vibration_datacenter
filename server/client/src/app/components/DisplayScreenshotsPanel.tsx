    ...
    <div style={{ display: "grid", gap: 12 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
        <div>
          ...
          <div style={{ color: C.textMuted, fontSize: "0.66rem", marginTop: 3 }}>
        ...
        >
          <RefreshCw size={13} style={{ opacity: loading ? 0.55 : 1 }} />
          Cập nhật
        </button>
      </div>

      {error ? (
        <div style={{ border: `1px solid ${C.danger}55`, background: C.dangerBg, color: C.danger, borderRadius: 8, padding: "10px 12px", fontSize: "0.68rem" }}>
          {error}
        </div>
      ) : null}
      ...
      {!loading && items.length === 0 ? (
        <div style={{ minHeight: 180, border: `1px dashed ${C.border}`, borderRadius: 10, display: "grid", placeItems: "center", color: C.textMuted, textAlign: "center", padding: 20 }}>
          <div>
            ...
            <div style={{ marginTop: 8, fontSize: "0.72rem", fontWeight: 800 }}>Chưa có client nào gửi ảnh.</div>
            <div style={{ marginTop: 4, fontSize: "0.64rem" }}>Mở dashboard trên màn hình cần giám sát và chờ khoảng 5 giây.</div>
          </div>
        </div>
      ) : (
          ...
          {items.map((item) => (
            ...
            >
              ...
              <div style={{ padding: 10 }}>
                <div style={{ color: C.textBright, fontSize: "0.74rem", fontWeight: 900, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  {item.displayName}
                </div>
                <div style={{ color: C.textMuted, fontSize: "0.62rem", marginTop: 5 }}>
                  {item.clientIp || "IP chưa xác định"} · {item.viewportWidth}×{item.viewportHeight} · DPR {item.devicePixelRatio}
                </div>
                <div style={{ color: C.textMuted, fontSize: "0.62rem", marginTop: 4 }}>
        ...
        >
          <div style={{ minHeight: 52, padding: "8px 14px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, background: C.surface, borderBottom: `1px solid ${C.border}` }}>
            <div style={{ minWidth: 0 }}>
              ...
              <div style={{ color: C.textMuted, fontSize: "0.63rem", marginTop: 3 }}>{selected.clientIp || "IP chưa xác định"} · {selected.pagePath}</div>
            </div>
            <button
              type="button"
          ...
          <div style={{ minHeight: 38, padding: "8px 14px", background: C.surface, borderTop: `1px solid ${C.border}`, color: C.textMuted, fontSize: "0.64rem", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>

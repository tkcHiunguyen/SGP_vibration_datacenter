...
export function ImportJobHistory({ jobs, activeJobId }: { jobs: ImportJob[]; activeJobId?: string }) {
  ...
  return (
    <div style={{ display: "grid", gap: 6 }}>
      <strong style={{ color: C.textBase, fontSize: "0.7rem" }}>Lịch sử import</strong>
        ...
        return (
          <div key={job.jobId} style={{ border: `1px solid ${job.jobId === activeJobId ? C.primary : C.border}`, borderRadius: 8, padding: "8px 9px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
            <span style={{ minWidth: 0 }}>
              <strong style={{ color: C.textBright, fontSize: "0.68rem", display: "block", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{job.fileName}</strong>
              <span style={{ color: C.textMuted, fontSize: "0.61rem" }}>{fmtDateTime(job.createdAt)} · {job.stage}</span>
            </span>
            <span style={{ display: "inline-flex", alignItems: "center", gap: 5, color, fontSize: "0.64rem", fontWeight: 850, flexShrink: 0 }}>

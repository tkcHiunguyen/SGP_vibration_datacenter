...
const ORDER: ImportStage[] = [
  ...
  "importing_placement_configs", "importing_spectrum", "rebuilding_summaries", "completed",
];
...
export function ImportJobProgress({ job, pollError }: { job: ImportJob; pollError?: string }) {
  ...
  return (
    <section style={{ border: `1px solid ${terminalError ? C.danger : C.cardBorder}`, borderRadius: 10, background: C.card, padding: 13, display: "grid", gap: 12 }}>
      ...
      <ProgressBar value={job.overallProgress ?? job.progress} color={terminalError ? C.danger : C.primary} background={C.surface} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 7 }}>
        {STEPS.map((step) => {
          ...
          return (
            <div key={step.stage} style={{ border: `1px solid ${state === "running" ? C.primary : C.border}`, borderRadius: 8, padding: "8px 9px", display: "flex", alignItems: "center", gap: 7, color }}>
        ...
        <div style={{ display: "grid", gap: 7 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 7, maxHeight: 220, overflow: "auto" }}>
            {job.devices.map((device) => {
              ...
              return (
                <div key={device.deviceId} style={{ border: `1px solid ${C.border}`, borderRadius: 8, padding: 9, display: "grid", gap: 6 }}>
                  ...
                  <ProgressBar value={progress} color={device.status === "failed" ? C.danger : C.success} background={C.surface} compact />
...
function ProgressBar({ value, color, background, compact = false }: { value: number; color: string; background: string; compact?: boolean }) {
  return (
    <div style={{ height: compact ? 5 : 9, borderRadius: 999, background, overflow: "hidden" }}>
      <div style={{ width: `${Math.max(0, Math.min(100, value))}%`, height: "100%", background: color, transition: "width 240ms ease" }} />
    </div>
  );
}
